...
bool Seleccion( int x, int y, Escena * escena, ContextoVis & cv_dib )
{
   ...
   if (id == 0) {
      cout << "Identificador de pixel es 0; no se hace nada" << endl;
      return false;
   }
   ...
   Objeto3D * objeto_aux;
   Tupla3f centro;
   if(objeto_raiz_act->buscarObjeto(id, MAT_Ident(), &objeto_aux, centro)){
      cam->mirarHacia(centro);
      cout << "Objeto con id " << id << " seleccionado: " + objeto_aux->leerNombre() << endl;
   }
   else{
